...
class PHISeg(AbstractHierarchicalProbabilisticSegmentationModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log GED^2
        ged = torch.zeros(len(all_ys), 3)
        y_pred_samples = harden_softmax_outputs(y_pred_samples, dim=2)
        for i in range(len(all_ys)):
            ys = convert_to_onehot(
                all_ys[i], num_classes=self.num_classes, channel_dim=1
            )
            ged[i] = torch.square(generalised_energy_distance(y_pred_samples[i], ys))
        ged = torch.mean(
            ged[:, 1:]
        )  # Only foreground and mean over classes and batches
        self.log("val/ged^2", torch.mean(ged), on_epoch=True)
