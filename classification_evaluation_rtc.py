...
def generating_datasets(
    ...
    rim_thickness_train = file_orig["train"]["rim_thickness"][()]
    labels_glaucoma_val = file_orig["val"]["diagnosis"][()]
    rim_thickness_val = file_orig["val"]["rim_thickness"][()]
    labels_glaucoma_test = file_orig_test["diagnosis"][()]
    rim_thickness_test = file_orig_test["rim_thickness"][()]
    rim_thickness_train_reshaped = rim_thickness_train.reshape((807, 5, 72, 10))
    x_train = rim_thickness_train_reshaped.mean(axis=-1).reshape((807 * 5, 72))
    y_train = labels_glaucoma_train.reshape((807 * 5))
    rim_thickness_val_reshaped = rim_thickness_val.reshape((202, 5, 72, 10))
    x_val = rim_thickness_val_reshaped.mean(axis=-1).reshape((202 * 5, 72))
    y_val = labels_glaucoma_val.reshape((202 * 5))
    rim_thickness_test_reshaped = rim_thickness_test.reshape((336, 5, 72, 10))
    x_test = rim_thickness_test_reshaped.mean(axis=-1).reshape((336 * 5, 72))
    ...
    rim_thickness_train_unet = file_unet["train"]["rim_thickness"][()]
    labels_glaucoma_val_unet = file_unet["val"]["majority_vote"][()]
    rim_thickness_val_unet = file_unet["val"]["rim_thickness"][()]
    labels_glaucoma_test_unet = file_unet_test["majority_vote"][()]
    rim_thickness_test_unet = file_unet_test["rim_thickness"][()]
    rim_thickness_unet_train_reshaped = rim_thickness_train_unet.reshape((807, 72, 10))
    x_train_unet = rim_thickness_unet_train_reshaped.mean(axis=-1).reshape((807, 72))
    y_train_unet = labels_glaucoma_train_unet.reshape((807))
    rim_thickness_val_unet_reshaped = rim_thickness_val_unet.reshape((202, 72, 10))
    x_val_unet = rim_thickness_val_unet_reshaped.mean(axis=-1).reshape((202, 72))
    y_val_unet = labels_glaucoma_val_unet.reshape((202))
    rim_thickness_unet_test_reshaped = rim_thickness_test_unet.reshape((336, 72, 10))
    x_test_unet = rim_thickness_unet_test_reshaped.mean(axis=-1).reshape((336, 72))
    ...
    for i in range(2, len(list_of_ds_paths_train)):
        ...
        # extract the train, val and test input + labels
        labels_glaucoma_train = generate_extended_labels(
            file_tr["train"]["majority_vote"][()], list_of_nsamples[i]
        )
        rim_thickness_train = file_tr["train"]["rim_thickness"][()]
        rim_thickness_train_reshaped = rim_thickness_train.reshape(
            (807, list_of_nsamples[i], 72, 10)
        )
        x_train = rim_thickness_train_reshaped.mean(axis=-1).reshape(
            (807 * list_of_nsamples[i], 72)
        )
